...
public class SandOreFeature extends OreFeature {

	public SandOreFeature(Codec<OreConfiguration> codec) {
		super(codec);
	}
												...
												if (level.ensureCanWrite(orePos)) {
													...
													if (section != null) {
														...
														for (OreConfiguration.TargetBlockState targetState : config.targetStates) {
															if (SandOreFeature.canPlaceOre(blockState, sectionGetter::getBlockState, random, config, targetState, orePos)) {
	...
	public static boolean canPlaceOre(@NotNull BlockState orePosState, @NotNull Function<BlockPos, BlockState> blockGetter, @NotNull RandomSource random, @NotNull OreConfiguration config, OreConfiguration.TargetBlockState targetState, BlockPos.@NotNull MutableBlockPos orePos) {
		if (!targetState.target.test(orePosState, random)) {
			return false;
		} else {
			return isAdjacentToWater(blockGetter, orePos);
		}
	}

	public static boolean isAdjacentToWater(Function<BlockPos, BlockState> blockGetter, BlockPos pos) {
		return checkNeighbors(blockGetter, pos, (b) -> (b.is(Blocks.WATER)));
	}
